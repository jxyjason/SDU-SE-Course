...
int main(int argc, char *argv[])
{
    ...
    consglue_key = 301; //消费者同步信号灯键值
    cmtxglue_key = 302; //消费者互斥信号灯键值
    ...
    consglue_sem = set_sem(cons_key,sem_val,sem_flg);

    //消费者互斥信号灯初值为1
    sem_val = 1;
    //获取消费者互斥信号灯，引用标识存pmtx_sem
    cmtxglue_sem = set_sem(cmtxglue_key,sem_val,sem_flg);
    ...
    while(1) {
        ...
        if(buff_ptr[*cget_ptr]==2) printf("%d consumer get: %c from Buffer[%d]\n", getpid(), buff_ptr[*cget_ptr], *cget_ptr);
