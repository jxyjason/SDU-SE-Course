...
int main(int argc, char *argv[])
{
    ...
    constoba_key = 301; //消费者同步信号灯键值
    cmtxtoba_key = 302; //消费者互斥信号灯键值
    ...
    cmtxtoba_sem = set_sem(cmtxtoba_key,sem_val,sem_flg);
    ...
    while(1) {
        ...
        if(buff_ptr[*cget_ptr]==0) printf("%d consumer get: %c from Buffer[%d]\n", getpid(), buff_ptr[*cget_ptr], *cget_ptr);
