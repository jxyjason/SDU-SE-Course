...
{ 
    ...
    if (freeMap->NumClear() < divRoundUp(numBytes, SectorSize))
...
FileHeader::setNumBytes(int numBytes)
{
    ...
    if(this->numBytes > numBytes)return FALSE;  
    if(this->numBytes == numBytes)return TRUE;
    ...
    if(nowNumSectors>NumDirect||freeMap->NumClear()< moreSectors)return FALSE;  
...
void
FileHeader::setNumSectors(int sectors){
    numSectors = sectors;
}
