...
// Here, condition variables are implemented using Hoare's style. We
//use semaphores to implement conditional variable. The algorithms is
//given in page 195 of the textbook. -ptang (aug 1995)

class Condition_H {
  public:
    Condition_H(const char* debugName);		// initialize condition to 
					// "no one waiting"
    ~Condition_H();			// deallocate the condition
    char* getName() { return (name); }
    
    void Wait(Semaphore *mutex, Semaphore *next, int *next_countPtr);
                                        // mutex and next are two
					// semaphores of the monitor ring,
					// next-count is the counter
					// for semaphore next.
