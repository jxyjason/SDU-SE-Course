...
OpenFile::~OpenFile()
{
    if (hdr->ifChanged)
    {
        time_t seconds;
        seconds = time(NULL);
        hdr->setNumSectors((int)seconds);
        printf("time:%d", (int)seconds);
        WriteBack();
        hdr->ifChanged = false;
...
void OpenFile::setHdrTime(int thetime)
{
    hdr->setNumSectors(thetime);
    hdr->ifChanged = false;
}
int OpenFile::getHdrTime()
{
    return hdr->getNumSectors();
}
...
int OpenFile::WriteAt(char *from, int numBytes, int position)
{
    ...
    {
        // numBytes = fileLength - position;
        if (hdr->setNumBytes(position + numBytes))
        {
            // printf("successfully extend\n");
            fileLength = hdr->FileLength();
        }
        else
        {
            printf("failed extend!\n");
        }
    }
